            ...
            $(data).each(function (index,genre) {
                tableBody.append('<tr>' +
                    ...
                    '<td> ' +
                        '<ul>' +
                            '<li>' +
                                '<button class="mt-1 main-button btn btn-sm btn-secondary" id="edit_'+genre.id+'" onclick="editGenre('+genre.id+')">Изменить</button>' +
                            '</li>'+
                            '<li>' +
...
function editGenre(genreId){
    var genre = window.location.href="/genre/"+genreId+"/edit";
